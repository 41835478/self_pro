...
class goodsControl extends sysControl{
	...
	public function goods_list(){
		...
		$Mgoods ->field('__AFFIX__goods.*,__AFFIX__community.name as community')
				...
				->where($where)
				->where($where2);
				if(!empty($area_goods_where)){
					$area_goods_where = array('comm_id' => 'in '.implode(',',$area_goods_where));
					$Mgoods->where($area_goods_where);
				}
	...
	public function goods_edit(){
		...
		if($community){
			foreach($community as & $val){
				$community_list[$val['id']] = $val['name'];
			}
		}
		...
		self::form("this",array(
			...
			array('radio','type','资源类别',array('1' => '出售' , '2' => '出租' , '3' => '非交易资源')),
			array('selected','comm_id','社区',$community_list),
			array('radio','is_show','是否显示',array('1' => '显示' , '2' => '不显示')),
	...
	private function commit(){
		$field = array('id','name','num','price','type','image','content','comm_id');
		...
		if(!empty($data) && $data['M'] == 'add'){
			...
			$log['other'] =  $_SESSION['admin']['username'].'在'.date('Y-m-d H:i:s').'时添加了资源,产品id是'.$res;
		}else if(!empty($data) && $data['M'] == 'update'){
			$u = M('goods')->where(array('id' => $_POST['id']))->find();
			$log['other'] = $_SESSION['admin']['username'].'在'.date('Y-m-d H:i:s').'时修改了资源,产品id是'.intval($_POST['id']);
		}
		M('admin_log')->add($log);
		if($res){
