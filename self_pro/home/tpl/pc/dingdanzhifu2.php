...
<body>
	...
	<a href="?act=payment&op=ali_room_pay&out_trade_no=<?php echo $output['out_trade_no'];?>&order_id=<?php echo $output['order']['order_id'];?>">
	<div class="dingdzf_4"  >
...
<script>
	...
	setInterval(function(){
		var url = '/api/api.php?commend=room_goods_is_buy';
		$.post(url,{order_id:order_id,out_trade_no:out_trade_no,sign:sign},function(data){
			if(data.msg.code == 1){
				window.location.href = '?act=order&op=order_over&order_id='+order_id;
			}
		},'json');
	},2000);
</script>