$(function () {
    ...
    $("#register-btn").click(function () {
        ...
        if (!checkMobile($phone)) {
            ...
            console.log($username,$userpassword,$phone,$captcha);
            $.ajax({
                type:'POST',
                url:baseurl + '/api/api.php?commend=phone_register',
                data:{phone:$phone,phone_code:$captcha,password:$userpassword,name:$username},
                success:function (data) {
                    console.log(data);
                    if (data.code == "-1")
                        alert(data.msg);
    ...
    $("#get-code2").click(function(){
        var $tel_num = $("#tel-num-input2").val();
        if (!checkMobile($tel_num)) {
            alert("请输入正确的手机号");
        } else {
            $.ajax({
    ...
    $("#findpwd-btn").click(function () {
        var $userpassword = $('#password-input2').val();
        var $phone = $('#tel-num-input2').val();
        var $captcha = $("#tel_number_code2").val();
        if (!checkMobile($phone)) {
            alert("请输入正确的手机号");
        } else if ($captcha.length < 1) {
            alert("请输入验证码");
            ...
            $.ajax({
                type:'POST',
                url:baseurl + '/api/api.php?commend=user_back',
                data:{phone:$phone,phone_code:$captcha,password:$userpassword},
                success:function (data) {
                    console.log(data);
                    if (data.code == "-1")
                        alert(data.msg);
