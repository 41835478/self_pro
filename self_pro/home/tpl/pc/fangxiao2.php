...
//        console.log(aaa);
	...
	$('.fangx2_songwan').on('click',function(){
		...
		var url = '/api/api.php?commend=fangjianxiaofeitianjiashangping2';
		$.post(url,{order_id:order_id,sign:sign,data:fangjianxiaofei,fujia:fujia},function(data){
			if(data.msg.code == '1'){
				window.location.href = '?act=payment&op=fangjianxiaofeizhifu&order_id=' + order_id + "&out_trade_no=" + data.msg.out_trade_no;
			}else{
				alert(data.msg.msg);
			}
		},'json');
