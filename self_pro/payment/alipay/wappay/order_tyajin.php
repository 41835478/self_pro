...
if (!empty($order)){
 ...
 //   $out_trade_no = trim($order['out_trade_no']);
    ...
    //支付宝交易号，和商户订单号二选一
    $trade_no = trim($order['transaction_id']);

    //退款金额，不能大于订单总金额
    $refund_amount=trim($order['yajin']);

    //退款的原因说明
    $refund_reason=trim($order['safe_name'].'押金退款');

    //标识一次退款请求，同一笔交易多次退款需要保证唯一，如需部分退款，则此参数必传。
    $out_request_no=trim($order['order_no']);
